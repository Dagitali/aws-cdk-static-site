...
| --- | --- | --- |
| Make maintenance contracts | `tests/unit/test_makefile.py` | Default suite |
...
docs-markdown` deterministically validates repository-local Markdown targets and heading anchors,
and is part of the default gate. `make docs-check` builds the deterministic strict HTML and EPUB
targets; external link checking remains the explicit `make docs-linkcheck` command because network
availability can be transient. `make test-installation` accesses the package index to install
runtime dependencies into clean virtual environments. `make test-security` installs the optional
security dependency group. The normal test command enforces branch coverage with a 90% minimum;
subprocess and optional layers use `--no-cov` and do not dilute the unit-test coverage measurement.
