...
- Threat, privacy, accessibility, migration, operability, and cost questions that benefit from a
...
- Reporting changed files, evidence, validation, operational effects, and follow-up.

Codex must not deploy, destroy, change DNS, publish, tag, change protected branches, or access
secrets unless the user explicitly authorizes that external action.
...
4. Compatibility, security, privacy, accessibility, data, cost, and migration constraints; and
...
5. Review the diff for unrelated changes, credential or confidential-evidence exposure, and
   documentation drift.
6. Record durable decisions in [Architecture Decision Records (ADRs)][ADRs] and reusable failures in
   `LEARNINGS.md`.
