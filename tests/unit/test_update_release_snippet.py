...
def readme_fixture(tmp_path: Path) -> Path:
...
class TestRender:
    ...
    def test_normalizes_release_tag(self, release: str) -> None:
...
class TestUpdate:
    ...
    def test_updates_only_marked_content(self, readme: Path) -> None:
    ...
    def test_accepts_current_snippet_in_check_mode(self, readme: Path) -> None:
        update(readme, '0.3.3')

        assert update(readme, 'v0.3.3', check=True) == []

    def test_rejects_stale_snippet_in_check_mode(self, readme: Path) -> None:
